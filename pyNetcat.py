...
def usage():
    ...
    print "-u --upload=destination -upon receiveing connection upload file and write to [destination]"
    print
    print
    print "Examples: "
    ...
    print "echo 'ABCDEFGHI' | ./pyNetcat.py -t 192.168.0.1 -p 135"
    sys.exit(0)
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hle:t:p:cu",["help","listen","exexute","target","port","command", "upload"])
    ...
    # listen and upload things, execute commands, and drop a shell back depending on cmdline  opts
    if listen:
        server_loop()

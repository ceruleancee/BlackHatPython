...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    # receive data from remote if any exist
    if receive_first:
        ...
        # send data to local if it exists
...
def hexdump(src, length=16):
    ...
    for i in xrange(0, len(src), length):
        s = src[i:i + length]
        hex = ' '.join(["%0*X" % (digits, ord(x)) for x in s])
...
def receive_from(connection):
    buffer = ""
    # set 2 second timeout, but ultimately depends on target
    connection.settimeout(2)
    try:
        # keep reading into the buffer until theres no data or timeout
        while True:
